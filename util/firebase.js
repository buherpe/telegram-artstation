...
const artwork = {
  async getArtworkFromDB() {
    ...
    try {
      ...
      logger.error('Error Getting Artwork in getArtworkFromDB: ' + error, "error");
    }
    return artwork;
  },
  ...
  async updateArtworkFromDB(hash, data) {
    ...
    try {
      ...
      logger.error('Error Updating Artwork in updateArtworkFromDB: ' + error, "error");
    }
    return statusUpdate;
  },
  async deleteArtworkFromDB(hash) {
    logger.info("🔴 при запросе проекта по хэшу " + hash + " произошла ошибка. элемент удален");
    try {
  ...
  async getArtworkList() {
    ...
    try {
      ...
      logger.info("🔴 при запросе проектов произошла ошибка");
    }
    return artworkList;
  },
    ...
    for (const media of assetsMedia) {
      telegram.sendPost(media).then(() => {
        this.updateArtworkFromDB(artworkFromDB.id, artworkFromDB).then((status) => {
          if (status) logger.info("🟢 пост " + artworkFromDB.id + " успешно отправлен.")
        })
      })
    }
  },
